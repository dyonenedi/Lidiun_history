	...
	Class Database
	{
		...
		static public function connect($con=null) {
			if (empty(self::$_con) || !empty($con)) {
				...
				if (!mysqli_connect_errno()) {
					...
					self::$_error = true;
					self::$_errorMessage = mysqli_connect_error();
		...
		*/
		static public function query($_sql,$return='boolean') {
			...
			if (self::$_result) {
				self::$_insertId = (!empty(self::$_con->_insert_id)) ? self::$_con->_insert_id : false;
				...
				self::$_error = true;
				self::$_errorMessage = self::$_con->_error.'<br>'.self::$_sql;
		...
		*/
		static public function autocommit($autocommit) {
			if ($autocommit) {
				...
				self::$_con->_autoCommit(true);
			} else {
				self::$_autoCommit = false;
				self::$_con->_autoCommit(false);
			}
		}
