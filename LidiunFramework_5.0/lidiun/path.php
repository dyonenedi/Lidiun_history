	...
	**********************************************************/
	...
	class Path
	{
		...
		public static function init() {
			...
			self::$_path['public'] = self::$_path['app'].'public/';
			...
			if (!empty(Conf::$_conf['path']['public_path']) && is_array(Conf::$_conf['path']['public_path'])) {
				foreach (Conf::$_conf['path']['public_path'] as $value) {
					$path = (substr($value, -1) == '/') ? $value : $value.'/';
					$path = (substr($path, 0, 1) == '/') ? substr($path, 1) : $path;
					$key = str_replace('/', ' ', $value);
					$key = str_replace(' ', '_', trim($value));
					self::$_path[$key] = self::$_path['public'].$path;
				}
			}
